...
- The job's `output_artifacts` field on `CompleteJob` SHOULD
  include the snapshot's `artifact_id` when the job's handler
  performed audited inference.
- Job kinds in the reverse-DNS namespace
  `cortex.synthesize.*`, `cortex.extract.*`, and `cortex.tool.*`
  are conventionally used for inference work; they are
